...
export default class PermissionController extends Controller {
  ...
  getMy = async ({ user }, res, next) => {
    try {
      ...
      const permissions = await postgresqlDB.get("permission", "role_id", user.role_id, fields);
  ...
  create = async ({ user, body }, res, next) => {
    try {
      ...
      res.json({ data: await postgresqlDB.create(this.entity, data, "*") });
    } catch (error) {
      next(error);
    }
  };
  ...
  deleteByCode = async ({ user, params }, res, next) => {
    try {
      ...
      if (!result.permitted) return next("FORBIDDEN");
      const { sql, values } = postgresqlDB.prepareQuery(this.deleteQuery, params);
      await postgresqlDB.query(sql, values);
